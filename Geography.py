...
class TGeography(GeneratedsSuper):
    ...
    def _hasContent(self):
        if (
            ...
            return False
    def export(self, outfile, level, namespaceprefix_='', namespacedef_='xmlns:es="http://www.EcoInvent.org/EcoSpold01"', name_='TGeography', pretty_print=True):
        ...
        self._exportAttributes(outfile, level, already_processed, namespaceprefix_, name_='TGeography')
        if self._hasContent():
            outfile.write('>%s' % (eol_, ))
            self._exportChildren(outfile, level + 1, namespaceprefix_, namespacedef_, name_='TGeography', pretty_print=pretty_print)
            outfile.write('</%s%s>%s' % (namespaceprefix_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def _exportAttributes(self, outfile, level, already_processed, namespaceprefix_='', name_='TGeography'):
    ...
    def _exportChildren(self, outfile, level, namespaceprefix_='', namespacedef_='xmlns:es="http://www.EcoInvent.org/EcoSpold01"', name_='TGeography', fromsubclass_=False, pretty_print=True):
        pass
