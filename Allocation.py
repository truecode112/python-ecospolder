...
class TAllocation(GeneratedsSuper):
    ...
    def validate_allocationMethodType(self, value):
        ...
        if value is not None and Validate_simpletypes_ and self.gds_collector_ is not None:
            ...
            if value < -1:
                lineno = self.gds_get_node_lineno_()
                self.gds_collector_.add_message('Value "%(value)s"%(lineno)s does not match xsd minInclusive restriction on allocationMethodType' % {"value": value, "lineno": lineno} )
                result = False
            if value > 2:
                lineno = self.gds_get_node_lineno_()
                self.gds_collector_.add_message('Value "%(value)s"%(lineno)s does not match xsd maxInclusive restriction on allocationMethodType' % {"value": value, "lineno": lineno} )
                result = False
    ...
    def _hasContent(self):
        if (
            ...
            return False
    def export(self, outfile, level, namespaceprefix_='', namespacedef_='xmlns:es="http://www.EcoInvent.org/EcoSpold01" xmlns:None="http://www.EcoInvent.org/EcoSpold01" ', name_='TAllocation', pretty_print=True):
        ...
        self._exportAttributes(outfile, level, already_processed, namespaceprefix_, name_='TAllocation')
        if self._hasContent():
            outfile.write('>%s' % (eol_, ))
            self._exportChildren(outfile, level + 1, namespaceprefix_, namespacedef_, name_='TAllocation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespaceprefix_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def _exportAttributes(self, outfile, level, already_processed, namespaceprefix_='', name_='TAllocation'):
        if self.referenceToCoProduct is not None and 'referenceToCoProduct' not in already_processed:
            already_processed.add('referenceToCoProduct')
            outfile.write(' referenceToCoProduct="%s"' % self.gds_format_integer(self.referenceToCoProduct, input_name='referenceToCoProduct'))
        if self.allocationMethod != -1 and 'allocationMethod' not in already_processed:
            already_processed.add('allocationMethod')
            outfile.write(' allocationMethod="%s"' % self.gds_format_integer(self.allocationMethod, input_name='allocationMethod'))
        if self.fraction is not None and 'fraction' not in already_processed:
            already_processed.add('fraction')
            outfile.write(' fraction="%s"' % self.gds_format_float(self.fraction, input_name='fraction'))
        if self.explanations is not None and 'explanations' not in already_processed:
            already_processed.add('explanations')
            outfile.write(' explanations=%s' % (self.gds_encode(self.gds_format_string(quote_attrib(self.explanations), input_name='explanations')), ))
    def _exportChildren(self, outfile, level, namespaceprefix_='', namespacedef_='xmlns:es="http://www.EcoInvent.org/EcoSpold01" xmlns:None="http://www.EcoInvent.org/EcoSpold01" ', name_='TAllocation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        for referenceToInputOutput_ in self.referenceToInputOutput:
            namespaceprefix_ = self.referenceToInputOutput_nsprefix_ + ':' if (UseCapturedNS_ and self.referenceToInputOutput_nsprefix_) else ''
            showIndent(outfile, level, pretty_print)
            outfile.write('<%sreferenceToInputOutput>%s</%sreferenceToInputOutput>%s' % (namespaceprefix_ , self.gds_format_integer(referenceToInputOutput_, input_name='referenceToInputOutput'), namespaceprefix_ , eol_))
    ...
    def _buildAttributes(self, node, attrs, already_processed):
        ...
        if value is not None and 'referenceToCoProduct' not in already_processed:
            already_processed.add('referenceToCoProduct')
            self.referenceToCoProduct = self.gds_parse_integer(value, node, 'referenceToCoProduct')
            self.validate_TIndexNumber(self.referenceToCoProduct)    # validate type TIndexNumber
        value = find_attr_value_('allocationMethod', node)
        if value is not None and 'allocationMethod' not in already_processed:
            already_processed.add('allocationMethod')
            self.allocationMethod = self.gds_parse_integer(value, node, 'allocationMethod')
            self.validate_allocationMethodType(self.allocationMethod)    # validate type allocationMethodType
