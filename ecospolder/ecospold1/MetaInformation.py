...
class TMetaInformation(GeneratedsSuper):
    ...
    def _hasContent(self):
        if (
            ...
            self.modellingAndValidation is not None or
            self.administrativeInformation is not None or
            self.anytypeobjs_
        ):
            return True
        else:
            return False
    def export(self, outfile, level, namespaceprefix_='', namespacedef_='xmlns:es="http://www.EcoInvent.org/EcoSpold01" xmlns:None="http://www.EcoInvent.org/EcoSpold01" ', name_='TMetaInformation', pretty_print=True):
        ...
        self._exportAttributes(outfile, level, already_processed, namespaceprefix_, name_='TMetaInformation')
        if self._hasContent():
            outfile.write('>%s' % (eol_, ))
            self._exportChildren(outfile, level + 1, namespaceprefix_, namespacedef_, name_='TMetaInformation', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespaceprefix_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def _exportAttributes(self, outfile, level, already_processed, namespaceprefix_='', name_='TMetaInformation'):
        pass
    def _exportChildren(self, outfile, level, namespaceprefix_='', namespacedef_='xmlns:es="http://www.EcoInvent.org/EcoSpold01" xmlns:None="http://www.EcoInvent.org/EcoSpold01" ', name_='TMetaInformation', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        if self.processInformation is not None:
            namespaceprefix_ = self.processInformation_nsprefix_ + ':' if (UseCapturedNS_ and self.processInformation_nsprefix_) else ''
            self.processInformation.export(outfile, level, namespaceprefix_, namespacedef_='', name_='processInformation', pretty_print=pretty_print)
        if self.modellingAndValidation is not None:
            namespaceprefix_ = self.modellingAndValidation_nsprefix_ + ':' if (UseCapturedNS_ and self.modellingAndValidation_nsprefix_) else ''
            self.modellingAndValidation.export(outfile, level, namespaceprefix_, namespacedef_='', name_='modellingAndValidation', pretty_print=pretty_print)
        if self.administrativeInformation is not None:
            namespaceprefix_ = self.administrativeInformation_nsprefix_ + ':' if (UseCapturedNS_ and self.administrativeInformation_nsprefix_) else ''
            self.administrativeInformation.export(outfile, level, namespaceprefix_, namespacedef_='', name_='administrativeInformation', pretty_print=pretty_print)
