...
class TValidation(GeneratedsSuper):
    ...
    def factory(*args_, **kwargs_):
        if CurrentSubclassModule_ is not None:
            subclass = getSubclassFromModule_(
                CurrentSubclassModule_, TValidation)
    ...
    def _hasContent(self):
        if (
            ...
            return False
    def export(self, outfile, level, namespaceprefix_='', namespacedef_='xmlns:es="http://www.EcoInvent.org/EcoSpold01"', name_='TValidation', pretty_print=True):
        ...
        self._exportAttributes(outfile, level, already_processed, namespaceprefix_, name_='TValidation')
        if self._hasContent():
            outfile.write('>%s' % (eol_, ))
            self._exportChildren(outfile, level + 1, namespaceprefix_, namespacedef_, name_='TValidation', pretty_print=pretty_print)
            outfile.write('</%s%s>%s' % (namespaceprefix_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def _exportAttributes(self, outfile, level, already_processed, namespaceprefix_='', name_='TValidation'):
        if self.proofReadingDetails is not None and 'proofReadingDetails' not in already_processed:
            already_processed.add('proofReadingDetails')
            outfile.write(' proofReadingDetails=%s' % (self.gds_encode(self.gds_format_string(quote_attrib(self.proofReadingDetails), input_name='proofReadingDetails')), ))
        if self.proofReadingValidator is not None and 'proofReadingValidator' not in already_processed:
            already_processed.add('proofReadingValidator')
            outfile.write(' proofReadingValidator="%s"' % self.gds_format_integer(self.proofReadingValidator, input_name='proofReadingValidator'))
        if self.otherDetails is not None and 'otherDetails' not in already_processed:
            already_processed.add('otherDetails')
            outfile.write(' otherDetails=%s' % (self.gds_encode(self.gds_format_string(quote_attrib(self.otherDetails), input_name='otherDetails')), ))
    def _exportChildren(self, outfile, level, namespaceprefix_='', namespacedef_='xmlns:es="http://www.EcoInvent.org/EcoSpold01"', name_='TValidation', fromsubclass_=False, pretty_print=True):
        pass
    ...
    def _buildAttributes(self, node, attrs, already_processed):
        ...
        if value is not None and 'proofReadingDetails' not in already_processed:
            ...
            self.validate_TString32000(self.proofReadingDetails)    # validate type TString32000
        value = find_attr_value_('proofReadingValidator', node)
        if value is not None and 'proofReadingValidator' not in already_processed:
            already_processed.add('proofReadingValidator')
            self.proofReadingValidator = self.gds_parse_integer(value, node, 'proofReadingValidator')
            self.validate_TIndexNumber(self.proofReadingValidator)    # validate type TIndexNumber
